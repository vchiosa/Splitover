...
var array3 =["fill","fill","fill","fill","fill","fill","fill"]  
    ...
    useEffect(() => {
        ...
        web3.eth.getAccounts((e,a)=>{
          ...
          contract.methods.getgameWinnersArray(referrer).call((e,r)=>{
            contract.methods.GameProcessed(referrer).call((e,p)=>{
              if(r.length==1){
                navigate("/success", { replace: true }); 
              }else if(r.length>1&&p==true){
                navigate("/success", { replace: true });
              }
              else if(r.length>1){
                if(r.indexOf(a[0])>=0){
    ...
    async function handleSubmit(num){
      try {
        contract.methods.selectNumber2(num,referrer).send({from:address});
...
console.log("winner array",winnerArray && winnerArray)
...
useEffect(()=>{
 ...
 setExtendedTime(currentTime+90)
    ...
    <div className='container_row' >
      <div className='row' >
        {winnerArray && winnerArray.map((v,e)=>{
 
          return(
          <div key={e} className='col-4'style={{width:"15%", marginTop:"40px"}}>
          <div className={`frame ${v==address? "other-fill" : "fill"}`}>
            <div id="glow">
              {array2[e].number}
            </div>
